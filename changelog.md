...
- Частично внедрена поддержка виртуального геймпада (Для корректной работы потребуется драйвер **ViGEmBus**).
В данный момент не работает.

### Изменено

- Логика разворачивания окна из трея (Пофиксил задержку перед появлением окна (не факт))
